...
class Detector(object):
    ...
    def build_detector(self):
        ...
        self.box_ph = tf.placeholder(shape=[None, None, 3],
                                       dtype=tf.float32, name='box_ph')
        ...
        img = tf.image.resize_bilinear(tf.expand_dims(self.image_ph, 0),
                                       (img_size, img_size))
        self.feature_net = self.net.create_trunk(img)
    ...
    def feed_forward(self, img, seg_gt, w, h, name, gt_bboxes, gt_cats, draw=False):
        ...
        feature = self.sess.run(self.feature_net,feed_dict=feed_dict)
